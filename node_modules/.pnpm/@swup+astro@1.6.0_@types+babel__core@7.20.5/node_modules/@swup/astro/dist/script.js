...
export function buildInitScript(options = {}) {
    let { accessibility = true, animationClass = 'transition-', cache = true, containers = ['main'], debug = false, forms = false, globalInstance = false, ignore = null, loadOnIdle = true, morph = false, parallel = false, preload = true, progress = false, reloadScripts = true, routes = false, smoothScrolling = true, theme = 'fade', updateBodyClass = true, updateHead = true, } = options;
    ...
    const dynamicImports = `
		const [${modules.join(', ')}] = await Promise.all([${imports.map(([, path]) => `import('${path}').then((m) => m.default)`).join(', ')}]);
	`;
